...
import { adminDeleteRoomReservationRequest, getAdminRoomReservationRequests, getRoomStatusLabel, type AdminRoomReservationRequest, type RoomReservationStatus } from '../../services/room-reservations';
...
export default function AdminRoomRequestsScreen() {
  ...
  const confirmDelete = (request: AdminRoomReservationRequest) => {
    Alert.alert('실습실 신청 삭제', `${request.requester?.name ?? '학생'}의 실습실 대여 신청을 삭제하시겠습니까?\n삭제하면 복구할 수 없습니다.`, [
      { text: '취소', style: 'cancel' },
      { text: '삭제', style: 'destructive', onPress: async () => {
        try {
          await adminDeleteRoomReservationRequest(request.id);
  ...
  return <SafeAreaView style={styles.safeArea} edges={['top']}>
    <StatusBar style="dark" /><View style={styles.header}><Pressable onPress={() => router.back()} hitSlop={10}><PlatformHeaderIcon name="back" /></Pressable><Text style={styles.headerTitle}>실습실 대여 관리</Text><View style={styles.headerSide} /></View>
...
const styles = StyleSheet.create({
  safeArea: { flex: 1, backgroundColor: COLORS.surface }, header: { height: 64, paddingHorizontal: 20, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderBottomWidth: 1, borderBottomColor: COLORS.border }, backText: { width: 40, color: COLORS.navy, fontSize: 38, lineHeight: 40 }, headerTitle: { color: COLORS.text, fontSize: 20, fontWeight: '800' }, headerSide: { width: 40 }, scrollView: { flex: 1, backgroundColor: COLORS.background }, content: { padding: 20, paddingBottom: 40 },
