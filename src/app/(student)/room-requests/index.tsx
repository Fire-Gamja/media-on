  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <Pressable onPress={() => confirmRoomRequestNavigation(() => router.push('/rooms'))} style={styles.addButton}><Text style={styles.addText}>신청</Text></Pressable>
      </View>
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={isRefreshing} onRefresh={() => void loadRequests(true)} colors={[COLORS.navy]} />}>
        {isLoading ? <View style={styles.stateBox}><ActivityIndicator size="large" color={COLORS.navy} /></View>
          ...
          : requests.length === 0 ? <View style={styles.stateBox}><Text style={styles.emptyTitle}>실습실 대여 신청이 없습니다.</Text><Pressable onPress={() => confirmRoomRequestNavigation(() => router.push('/rooms'))} style={styles.retryButton}><Text style={styles.retryText}>실습실 둘러보기</Text></Pressable></View>
...
const styles = StyleSheet.create({
  safeArea: { flex: 1, backgroundColor: COLORS.surface }, header: { height: 64, paddingHorizontal: 20, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderBottomWidth: 1, borderBottomColor: COLORS.border }, backText: { width: 40, color: COLORS.navy, fontSize: 38, lineHeight: 40 }, headerTitle: { color: COLORS.text, fontSize: 20, fontWeight: '800' }, addButton: { minWidth: 44, height: 36, alignItems: 'center', justifyContent: 'center', borderRadius: 10, backgroundColor: COLORS.navy }, addText: { color: COLORS.white, fontSize: 13, fontWeight: '800' },
