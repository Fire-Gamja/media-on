...
export default function EquipmentRequestDetailScreen() {
  ...
  useEffect(() => {
    ...
    void getMyEquipmentRentalRequest(requestId)
      .then(setRequest)
      .catch((error) => {
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>대여 신청 상세</Text>
      ...
      {isLoading ? (
        ...
        <ScrollView style={styles.scrollView} contentContainerStyle={styles.content}>
          <View style={styles.statusCard}>
            ...
            <Text style={styles.statusValue}>{getEquipmentStatusLabel(request.status)}</Text>
            <Text style={styles.statusDescription}>{getStatusDescription(request)}</Text>
          </View>

          <View style={styles.card}>
            <DetailRow label="기자재" value={request.equipment?.name ?? '기자재'} />
            <DetailRow label="신청 수량" value={`${request.quantity}개`} />
            <DetailRow label="대여일" value={request.pickup_date} />
            <DetailRow label="반납일" value={request.return_date} />
          ...
          <View style={styles.noteCard}>
            <Text style={styles.noteTitle}>
              {request.status === 'rejected' ? '반려 사유' : '관리자 안내'}
            </Text>
            <Text style={styles.noteText}>
              {request.admin_note?.trim() || getDefaultNote(request.status)}
            </Text>
          </View>
        </ScrollView>
      ) : null}
...
function getStatusDescription(request: EquipmentRentalRequest) {
  if (request.status === 'submitted') return '관리자가 신청 내용을 확인 중입니다.';
  if (request.status === 'approved') return `${request.pickup_date}에 수령해 주세요.`;
  if (request.status === 'checked_out') return `${request.return_date}까지 반납해 주세요.`;
  if (request.status === 'returned') return '기자재 반납이 완료되었습니다.';
  return '신청이 반려되었습니다. 아래 사유를 확인해 주세요.';
}

function getDefaultNote(status: EquipmentRentalRequest['status']) {
  if (status === 'submitted') return '대여 승인 결과를 기다려 주세요.';
  if (status === 'approved') return '승인이 완료되었습니다. 학부 사무실에서 수령해 주세요.';
  if (status === 'checked_out') return '기자재를 안전하게 사용한 뒤 기한 내 반납해 주세요.';
  if (status === 'returned') return '반납 확인이 완료되었습니다.';
...
const styles = StyleSheet.create({
  ...
  statusValue: { marginTop: 8, color: COLORS.white, fontSize: 23, fontWeight: '900' },
  statusDescription: { marginTop: 10, color: '#D9DDEF', fontSize: 12, lineHeight: 19 },
  ...
  purposeSection: { paddingTop: 17, borderTopWidth: 1, borderTopColor: COLORS.border },
