...
export default function AssistantInquiryDetailScreen() {
  ...
  return <SafeAreaView style={styles.safeArea} edges={['top']}><StatusBar style="dark" /><View style={styles.header}><Pressable onPress={() => router.back()} hitSlop={10}><PlatformHeaderIcon name="back" /></Pressable><Text style={styles.headerTitle}>조교 문의 상세</Text><View style={styles.headerSide} /></View>
    {isLoading ? <View style={styles.loadingBox}><ActivityIndicator size="large" color={COLORS.navy} /></View> : inquiry ? <View style={styles.chatScreen}>
      <AssistantChatRoom
        header={<>
          ...
          <Text style={styles.answerTitle}>실시간 상담</Text>
        </>}
        inquiryId={inquiry.id}
        status={inquiry.status}
        onStatusChange={(status: AssistantInquiryStatus) => setInquiry((current) => current ? { ...current, status } : current)}
      />
    </View> : null}
  </SafeAreaView>;
}
...
const styles = StyleSheet.create({
  safeArea: { flex: 1, backgroundColor: COLORS.surface }, header: { height: 64, paddingHorizontal: 20, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderBottomWidth: 1, borderBottomColor: COLORS.border }, backText: { width: 40, color: COLORS.navy, fontSize: 38, lineHeight: 40 }, headerTitle: { color: COLORS.text, fontSize: 18, fontWeight: '800' }, headerSide: { width: 40 }, loadingBox: { flex: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: COLORS.background }, chatScreen: { flex: 1, backgroundColor: COLORS.background }, statusCard: { padding: 20, borderRadius: 18, backgroundColor: COLORS.navy }, statusLabel: { color: '#D9DDEF', fontSize: 12, fontWeight: '700' }, statusValue: { marginTop: 8, color: COLORS.white, fontSize: 23, fontWeight: '900' }, statusDescription: { marginTop: 10, color: '#D9DDEF', fontSize: 12, lineHeight: 19 }, card: { marginTop: 16, padding: 19, borderWidth: 1, borderColor: COLORS.border, borderRadius: 18, backgroundColor: COLORS.surface }, title: { color: COLORS.text, fontSize: 18, lineHeight: 26, fontWeight: '800' }, date: { marginTop: 8, color: COLORS.placeholder, fontSize: 11 }, contentSection: { marginTop: 18, paddingTop: 17, borderTopWidth: 1, borderTopColor: COLORS.border }, sectionLabel: { color: COLORS.subText, fontSize: 12, fontWeight: '800' }, bodyText: { marginTop: 10, color: COLORS.text, fontSize: 14, lineHeight: 23 }, answerTitle: { marginTop: 18, color: COLORS.navy, fontSize: 15, fontWeight: '900' },
});
